...
- [ ] Anti-aliasing toggle - Butterchurn already exposes `setOutputAA(bool)` directly, basically a one-line addition
- [ ] Render/canvas resolution scale - distinct from Mesh Size (the warp-grid resolution): render at e.g. 0.5x-2x the display size, trading sharpness for performance
- [ ] Beat-driven auto-cycle - advance presets on detected bass hits instead of (or alongside) the fixed Cycle Interval timer, using the audio analysis already feeding the visualizer
...
- [ ] MilkDrop preset compatibility - `repairBadShader()` fixes the
